...
from da4mt.splitting import k_fold_scaffold_split, random_split
...
def make_splits(args):
    ...
    # Splits dataset
    for splitter, n in zip(args.splitter, args.num_splits, strict=False):
        if splitter == "scaffold":
            ...
            raise NotImplementedError(f"Splitter {splitter} not implemented.")

        logger.info("Successfully created splits.")

        for i, split in enumerate(splits):
            with open(args.output_dir / f"{filename}_{i}.{splitter}_splits.json", "w") as f:
